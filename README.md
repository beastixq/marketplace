...
Веб-приложение онлайн-маркетплейса — платформы, где независимые продавцы размещают товары, а покупатели выбирают и заказывают их через единый каталог. Система обеспечивает управление товарами, оформление заказов, оставление отзывов и аналитику продаж. Серверная часть реализована на Go; слой доступа к данным поддерживает PostgreSQL и MongoDB с переключением через конфигурацию. Redis используется как опциональный кэш.
...
| Встроенная аналитика продаж | Расширенная аналитика в личном кабинете, часть функций доступна по подписке [Ozon Premium](https://docs.ozon.ru/api/seller/) | Базовая аналитика; для расширенной требуются сторонние сервисы ([обзор сервисов](https://vc.ru/marketplace/2035503-luchshie-servisy-analitiki-marketpleysov-2025)) | Базовая аналитика в кабинете продавца                                                                                             | Статистика через repository layer, доступ по REST API                         |
...
2. Система отображает список товаров категории (данные из выбранного хранилища: PostgreSQL или MongoDB).
...
4. Система сохраняет товар в выбранное хранилище.
...
7. В PostgreSQL изменение цены записывает триггер; в MongoDB эквивалентную запись в `product_price_history` создаёт MongoDB-репозиторий.
8. Продавец открывает раздел «Статистика продаж», указывает период.
9. Система возвращает количество заказов, общую выручку, средний чек и самый продаваемый товар через общий service/repository контракт.
...
| РСУБД              | PostgreSQL 16          | Основной реляционный режим хранения данных         |
| Документная СУБД   | MongoDB 8              | Альтернативный режим хранения данных для ЛР9       |
| Драйвер БД         | pgx/v5 + pgxpool       | Пул соединений с PostgreSQL                        |
| MongoDB driver     | mongo-driver/v2        | Подключение к MongoDB, индексы, sessions, tx       |
...
## Переключение хранилища для ЛР9

Приложение выбирает реализацию repository layer через `database.type`:

```yaml
database:
  type: "postgres" # postgres | mongo
...
MongoDB запускается как single-node replica set, потому что транзакции MongoDB работают через sessions и требуют replica set. Данные PostgreSQL в MongoDB не мигрируются: MongoDB-режим стартует с пустыми коллекциями, а необходимые индексы создаются приложением при подключении.

Для удобного доступа MongoDB-режим при первом подключении идемпотентно создаёт те же системные аккаунты, что PostgreSQL migration `008_system_accounts.sql`:

| Роль | Email | Пароль |
| --- | --- | --- |
| Admin | `admin@marketplace.local` | `admin123` |
| Analyst | `analyst@marketplace.local` | `analyst123` |
...
Реализации и DB entities: PostgreSQL-пакет `repository`, MongoDB-пакет `repository/mongo`, общий компонент сборки `component/repository`:
