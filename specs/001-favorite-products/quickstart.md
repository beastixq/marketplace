...
```bash
curl -i -X POST "http://localhost:8080/api/v1/products/${PRODUCT_ID}/favorite" \
...
- `204 No Content` when the product was already favorited.
- `409 Conflict` with `{"error":"Product is deleted"}` or equivalent service
  error when the product is not active and visible.

## Check Favorite State

```bash
curl -s "http://localhost:8080/api/v1/products/${PRODUCT_ID}/favorite" \
  -H "Authorization: Bearer ${TOKEN}"
```
...
```bash
curl -s "http://localhost:8080/api/v1/users/me/favorites?page=1&limit=20" \
  -H "Authorization: Bearer ${TOKEN}"
```
...
```bash
curl -i -X DELETE "http://localhost:8080/api/v1/products/${PRODUCT_ID}/favorite" \
  -H "Authorization: Bearer ${TOKEN}"
```
