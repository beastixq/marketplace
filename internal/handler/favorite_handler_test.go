...
func TestFavoriteHandler_AddFavorite(t *testing.T) {
	...
	t.Run("Invalid product id", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPut, "/api/v1/favorites/bad", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
		router.ServeHTTP(rr, req)
	...
	t.Run("Inactive product", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPut, "/api/v1/favorites/99", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
		router.ServeHTTP(rr, req)
...
func TestFavoriteHandler_RemoveListAndCheck(t *testing.T) {
	...
	t.Run("Check favorite state", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/api/v1/favorites/42", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
		router.ServeHTTP(rr, req)
		if rr.Code != http.StatusOK {
	...
	t.Run("List favorite products", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/api/v1/favorites?page=1&limit=10", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
		router.ServeHTTP(rr, req)
		if rr.Code != http.StatusOK {
	...
	t.Run("Remove favorite", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodDelete, "/api/v1/favorites/42", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
		router.ServeHTTP(rr, req)
