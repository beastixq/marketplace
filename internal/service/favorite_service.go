...
// validate that product favorites only target active, visible products.
...
func (fs FavoriteService) RemoveFavorite(ctx context.Context, actor Actor, productID int64) error {
	if err := fs.productExists(ctx, productID); err != nil {
		return err
	}
...
func (fs FavoriteService) productExists(ctx context.Context, productID int64) error {
	_, err := fs.productGetter.GetProductByID(ctx, productID)
	if err != nil {
		if errors.Is(err, ErrNotFound) {
