...
- The repository interface owns the contract; the implementation does not. Document each method's semantics (return-value meaning, error sentinels, idempotency, what callers may NOT infer) as a Go doc comment on the *interface method* in `internal/service`. Implementations in `internal/repository` may add a short "how" comment naming the SQL strategy (e.g. "implements ... using INSERT ... ON CONFLICT ...") but must not redocument the contract: if interface and implementation comments drift, the interface wins. This mirrors `io.Reader` / `*os.File.Read`: the interface declares what `Read` means; the file's `Read` only describes the file-specific mechanics.
